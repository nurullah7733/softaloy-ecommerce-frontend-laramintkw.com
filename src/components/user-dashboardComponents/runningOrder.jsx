...
const RunningOrders = () => {
  ...
  const handleClick = async (id) => {
    const confirmBtnResult = await randomSweetAlert(
      "Are you sure?",
      "You won't be able to revert this!"
    );
    if (confirmBtnResult.isConfirmed) {
      const result = await changeOrderStatusRequest(id, {
        ...
        {data?.rows?.map((indivitualOrder, index) => (
          <div key={index} className="border p-2 shadow-md">
            ...
            <p>
              ...
              <button
                className="px-2 rounded-sm text-black font-bold bg-gray-200"
        ...
        <div>
          <div>
            <h1 className="w-full md:text-xl dark:bg-gray-700 px-5 py-3.5 text-3xl border-b text-black  ">
              {data?.rows?.length > 0 && ` (${data?.rows?.length})`} Running{" "}
