...
import { deliveryOrdersRequest } from "../../../APIRequest/orderApi";
...
export default function DeliveryOrders() {
  ...
  useEffect(() => {
    (async () => {
      var allData = await deliveryOrdersRequest();
      setData(allData);
    })();
  }, []);
  ...
  if (data?.total?.length < 1) {
    finalData = <h1 className="px-5 text-2xl">No Delivery orders</h1>;
  } else {
    finalData = (
  ...
  return (
    <div className="mx-auto px-8 pt-10 pb-20">
      <h1 className="w-full text-center md:text-xl dark:bg-gray-700 px-5  text-3xl border-l text-white bg-primary">
        Running orders
      </h1>
