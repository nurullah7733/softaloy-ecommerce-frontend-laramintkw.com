...
import { returnOrdersRequest } from "../../APIRequest/orderApi";
...
const ReturnOrder = () => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      var allData = await returnOrdersRequest();
      setLoading(false);
      setData(allData);
    })();
  }, []);
  ...
  if (data?.total?.length < 1) {
    finalData = <h1 className="px-5 text-2xl">No Return orders</h1>;
  } else {
    finalData = (
        ...
        <div>
          <div>
            <h1 className="w-full md:text-xl dark:bg-gray-700 px-5 py-3.5 text-3xl border-b text-black  ">
              {data?.rows?.length > 0 && ` (${data?.rows?.length})`} Return{" "}
